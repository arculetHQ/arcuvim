...
function M.live_multigrep(opts)
	if not ensure_rg_available() then
		return
	end
	...
	local picker = pickers.new(opts, {
		...
		attach_mappings = function(prompt_bufnr, map)
			...
			map("i", "<C-h>", function()
				...
				local help_text = "Usage: <search_term>"
					.. separator
					.. "<file_pattern> (separate with "
					.. separator_desc
					.. ")"
...
function M.buffer_multigrep(opts)
	...
	local entry_maker = function(entry)
		return {
			value = entry,
			display = string.format("%s:%d:%d %s", entry.filename, entry.lnum, entry.col, entry.text),
			ordinal = entry.text,
...
function M.setup(user_config)
	...
	if M.config.mappings ~= false then
		...
		-- Basic multi-grep
		vim.keymap.set("n", mappings.multigrep, function()
			M.live_multigrep()
		end, { desc = "Telescope multi-grep (pattern + file filter)" })

		-- Project-wide search
		if mappings.project_grep ~= false then
			vim.keymap.set("n", mappings.project_grep, function()
				M.live_multigrep({
					prompt_title = M.config.prompt_titles.project,
					cwd = vim.fn.getcwd(),
				})
			end, { desc = "Telescope multi-grep in project root" })
		end

		-- Hidden files search
		if mappings.hidden_grep ~= false then
			vim.keymap.set("n", mappings.hidden_grep, function()
				M.live_multigrep({
					prompt_title = M.config.prompt_titles.hidden,
					hidden = true,
				})
			end, { desc = "Telescope multi-grep including hidden files" })
		end

		-- Buffer search
		if mappings.buffer_grep ~= false then
			vim.keymap.set("n", mappings.buffer_grep, function()
				M.buffer_multigrep({
					prompt_title = M.config.prompt_titles.buffer,
				})
			end, { desc = "Telescope multi-grep in open buffers" })
		end
	end
